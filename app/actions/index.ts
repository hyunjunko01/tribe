...
export const signInAction = async (formData: FormData) => {
  ...
  // Manually created auth users may have a profile but no Circle wallet.
  // Provision one on first login so the dashboard does not crash.
  try {
    ...
    if (!existingWallet) {
      ...
      if (!createdWalletSet?.id) {
        console.error("Failed to create wallet set on sign-in:", createdWalletSet);
        return encodedRedirect(
          "error",
          "/sign-in",
          "Signed in, but wallet setup failed. Please try again.",
        );
      }
      ...
      if (!createdWallet?.id) {
        console.error("Failed to create wallet on sign-in:", createdWallet);
        return encodedRedirect(
          "error",
          "/sign-in",
          "Signed in, but wallet setup failed. Please try again.",
      ...
      if (walletError) {
        console.error("Error while inserting wallet on sign-in:", walletError);
        return encodedRedirect(
          "error",
          "/sign-in",
          "Signed in, but wallet setup failed. Please try again.",
    ...
    console.error("Wallet provisioning on sign-in failed:", provisionError?.message);
    return encodedRedirect(
      "error",
      "/sign-in",
      "Signed in, but wallet setup failed. Please try again.",
