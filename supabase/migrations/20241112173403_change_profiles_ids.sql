...
SELECT 
    extensions.uuid_generate_v4(), -- Generate new UUID for profile
    temp_profiles.id,                -- Use existing id as auth_user_id
    temp_profiles.name,
    temp_profiles.avatar_url,
    temp_profiles.created_at,
    temp_profiles.updated_at,
    temp_profiles.is_active
FROM temp_profiles;
