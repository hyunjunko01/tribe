...
Last updated: 2026-07-23 (arbiter/admin dispute planned; store resolve is temporary)
...
| 3a+   | 7        | Arbiter / admin dispute resolve        | **Next**     |
| 3b    | 8        | Integrator docs + demo polish          | Planned      |
...
```
 ...
 Dispute resolve (`refund` | `release`) — implemented for **store** (Phase 6, temporary).
 Admin / arbiter resolve — **planned (Phase 7)**.
...
| Resolve dispute        | `POST /api/orders/[id]/resolve`       | Store today (`refund` \| `release`); move to admin in Phase 7 |
...
| -------------------- | --------------------------------- | ------------------------------------------ |
| Admin resolve dispute | `POST /api/orders/[id]/resolve` (admin-only) or `/api/admin/orders/[id]/resolve` | Arbiter; prefer `refundByArbiter` where useful |
| List disputed orders | `GET /api/admin/orders?status=DISPUTED` | Admin dashboard                            |
...
> **Note:** Phase 6 store resolve is a **temporary demo**. Prefer admin/arbiter in Phase 7; then hide store resolve actions.

### Phase 7 — Arbiter / admin dispute ← **NEXT**


| ✓   | Task                                                              | Commit | Suggested message                                              |
| --- | ----------------------------------------------------------------- | ------ | -------------------------------------------------------------- |
| [ ] | Admin auth gate (who may act as arbiter)                          | —      | `Add admin gate for dispute resolve.`                          |
| [ ] | Admin resolve API (only admin; `refund` \| `release`)             | —      | `Restrict dispute resolve to admin arbiter.`                   |
| [ ] | Prefer `refundByArbiter` for admin refund path                    | —      | `Use arbiter wallet for disputed order refunds.`               |
| [ ] | `/dashboard/admin` — list `DISPUTED` orders + resolve buttons     | —      | `Add admin dashboard for disputed order resolve.`              |
| [ ] | Hide store resolve buttons; store can view disputed only          | —      | `Move dispute resolve actions from store to admin dashboard.`  |
...
| 5     | UI split              | **Done**                       |
| 6     | Refund & dispute      | **Done** (store resolve = temporary) |
| 7     | Arbiter / admin dispute | **Next** ← pick up here      |
| 8     | Integrator docs       | Planned                        |
...
| **7**    | Arbiter resolve  | `DISPUTED` → **admin/arbiter** refund or release; store cannot settle                                    | **Next** |
| **8**    | Integrator story | Recorded E2E demo + integration doc; can pitch “middleware skeleton”                                     | Planned  |
...
5. **Unhappy path:** Bad photo → `DISPUTED` → **admin/arbiter** resolves refund to customer  
   *(Today’s store resolve buttons are temporary until Phase 7.)*
...
| Admin dispute UI       | `app/dashboard/admin/` *(planned Phase 7)*                       |
...
**Next code commit:** Phase 7 — `Add admin gate for dispute resolve.`