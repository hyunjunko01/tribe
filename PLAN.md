...
| ✓ | Task | Commit | Suggested message |
|---|------|--------|-------------------|
| [ ] | Add `/dashboard/customer` route (shell + wallet) | — | `Add customer dashboard route.` |
...
| [ ] | `POST /api/orders/[id]/resolve` (`refund` \| `release`) | — | `Add dispute resolve endpoint for disputed orders.` |
...
Why not merchant? Payment/PG jargon (Stripe-style). Our domain is delivery — “가게(store)” is clearer and matches the schema (`stores`, `storeId`, `store_profile_id`).

Example commit messages:
- `Add store registry and restrict orders to registered stores.`
- `Add PLAN.md for project roadmap and agent onboarding.`
